...
import { Stethoscope, FileText, Activity, Sparkles, AlertTriangle } from "lucide-react";
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (docData?.doctor_id) {
        const { data: patientsData } = await supabase.from("patients").select("*").eq("assigned_doctor_id", docData.doctor_id).order("created_at", { ascending: false });
            ...
            <div className="lg:col-span-1 space-y-4">
              ...
              {alerts.length > 0 ? (
                <div className="space-y-4">
                  {alerts.map((alert, i) => (
                    <div key={i} className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl p-4 text-white shadow-md shadow-purple-500/20">
                  ...
                  <div className="divide-y divide-gray-100">
                    {patients.map((p) => (
                      <div key={p.patient_id} className="p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                        ...
                        <Link to={`/patient/${p.patient_id}`} className="px-4 py-2 bg-purple-50 hover:bg-purple-100 text-purple-700 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors">
